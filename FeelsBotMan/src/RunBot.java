...
public class RunBot {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		// Initialize BotResponse list
		try {
			BotResponseStage.initializeResponseStage();
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		for (int j = 0; j < trials.size(); j++) {
			...
			while (speech_results == null || !correct) {
				...
				// determine whether normal protocol can continue, or if special protocols
				// (anger, anxiety) must be activated
				String response_type = whichProtocol(expressions, speech_results);

				System.out.println("User sentiment: " + user_sentiment);

				// take weighted average of facial expression scores and text analysis scores to
				// get overall sentiment score
				double score = reconcileScore(expressions, (double) user_sentiment.get(0));

				correct = trial.determine_response_correctness((String) user_sentiment.get(2), speech_results);
				System.out.println("Correctness: " + correct + "\n");

				// determine bot's appropriate response
				String response = "Good job! You're done with this training set! :D";
				if (!correct || j != trials.size() - 1) {
					response = BotResponseStage.determineBotResponse(score, response_type, correct);
				}
	...
	public static ArrayList<Double> facialAnalysis() {
		/*
		 * Will eventually call vision.py detect_face() method; for now, just returns
		 * something that will let the rest of the program run
		 */
